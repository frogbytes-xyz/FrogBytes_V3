...
export async function processSummarizationJob(
  ...
  try {
    ...
    if (fetchError || !transcriptionData) {
      throw new Error(
        'Transcription not found. It may have been deleted or does not exist'
      )
    }
    ...
    if (configured) {
      ...
      logger.warn(
        '[Worker] Gemini API key not configured, using mock summarization'
      )
    ...
    const titleMatch = summResult.latexContent.match(
      /\\(?:section|chapter)\{([^}]+)\}/
    )
    const extractedTitle = titleMatch
      ? titleMatch[1]
      : `Summary ${job.summaryType}`

    // Save summary to database
    const { data: summary, error: insertError } = await (
      supabase.from('summaries') as any
    )
      .insert({
    ...
    if (insertError) {
      throw new Error(
        `Failed to save summary to database: ${insertError.message}`
      )
    }
    ...
    if (!pdfResult.success) {
      ...
      logger.info('[Worker] PDF URL:', { pdfUrl: pdfResult.pdfUrl })
      logger.info('[Worker] Archive link:', {
        archiveLink: pdfResult.telegramArchiveLink
      })
      logger.info('[Worker] PDF link:', { pdfLink: pdfResult.telegramPdfLink })
    }

    const result: SummarizationJobResult = {
      success: true,
      summaryId: summary.id
    }
    if (pdfResult.pdfUrl) result.pdfUrl = pdfResult.pdfUrl
    if (pdfResult.telegramArchiveLink)
      result.telegramArchiveLink = pdfResult.telegramArchiveLink
    if (pdfResult.telegramPdfLink)
      result.telegramPdfLink = pdfResult.telegramPdfLink
    return result
  } catch (error) {
    const errorMessage =
