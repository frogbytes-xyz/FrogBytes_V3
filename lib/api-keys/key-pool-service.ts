...
export async function getKeyForTextGeneration(): Promise<string | null> {
  try {
    ...
    const { data, error } = await supabase
      ...
      .select(
        'api_key, can_generate_text, last_validated_at, success_count, quota_count'
      )
      .eq('status', 'valid')
    ...
    logger.info(
      `Selected key for text generation: ${data.api_key.substring(0, 12)}...`
    )
    return data.api_key
  } catch (error: any) {
 ...
 */
export async function getKeysForParallelProcessing(
  count: number
): Promise<string[]> {
  try {
    const supabase = getSupabaseClient()
    ...
    const { data, error } = await supabase
      .from('working_gemini_keys')
      .select(
        'api_key, can_generate_text, max_tokens, success_count, quota_count'
      )
      .eq('status', 'valid')
      .eq('can_generate_text', true)
 ...
 */
export async function getKeyForModel(
  modelName: string
): Promise<string | null> {
  try {
    const supabase = getSupabaseClient()
    ...
    for (const key of data) {
      ...
      if (hasModel) {
        logger.info(
          `Found key for model ${modelName}: ${key.api_key.substring(0, 12)}...`
        )
        return key.api_key
      }
    }
...
export async function markKeyInvalid(
  ...
  try {
    ...
    await supabase.from('working_gemini_keys').delete().eq('api_key', apiKey)

    logger.warn(
      `Marked key as invalid and removed: ${apiKey.substring(0, 12)}...`
    )
  } catch (error: any) {
...
export async function getPoolStats(): Promise<{
  ...
  try {
    ...
    const [allPotentialRes, pendingPotentialRes, workingResult] =
      await Promise.all([
        supabase
          .from('potential_keys')
    ...
    return {
      ...
      quotaExceededKeys: working.filter(k => k.status === 'quota_exceeded')
        .length,
      pendingValidation,
      textGenerationKeys: working.filter(
        k => k.status === 'valid' && k.can_generate_text
      ).length,
      imageGenerationKeys: working.filter(
        k => k.status === 'valid' && k.can_generate_images
      ).length
    }
  } catch (error: any) {
...
export async function getKeyDetails(
  apiKey: string
): Promise<KeyWithCapabilities | null> {
  try {
    const supabase = getSupabaseClient()
