...
class KeyScraperService {
  ...
  stop() {
    this.running = false
    scraperLogger.always(
      `Stopped. Total scraped: ${this.stats.totalScraped}, saved: ${this.stats.totalSaved}`
    )
  }
  ...
  private async testConnection() {
    ...
    if (error) {
      throw new Error(
        `Database table 'potential_keys' not accessible: ${error.message}`
      )
    }
  }
  ...
  private async loadExistingKeys() {
    try {
      scraperLogger.always(
        `[INIT] Loading existing keys from database to prevent duplicates...`
      )
      ...
      if (potentialResult.error) {
        scraperLogger.error(
          `[INIT] Error loading potential_keys: ${potentialResult.error.message}`
        )
      } else if (potentialResult.data) {
        potentialResult.data.forEach((row: any) =>
          this.seenKeys.add(row.api_key)
        )
      ...
      if (workingResult.error) {
        scraperLogger.error(
          `[INIT] Error loading working_gemini_keys: ${workingResult.error.message}`
        )
      ...
      scraperLogger.always(
        `[INIT] [SUCCESS] Loaded ${this.seenKeys.size} existing keys (${potentialCount} potential, ${workingCount} working)`
      )
      scraperLogger.always(
        `[INIT] Scraper will skip any keys already in this set`
      )
    } catch (error: any) {
      scraperLogger.error(
        `[INIT] Error loading existing keys: ${error.message}`
      )
      scraperLogger.error(
        `[INIT] Continuing without duplicate checking - database will handle it`
      )
    }
  }
  ...
  private async scrapeLoop() {
    ...
    while (this.running) {
      if (
        ...
        Date.now() - this.stats.startTime.getTime() > this.config.maxDuration
      ) {
        break
      }
      ...
      try {
        ...
        if (iteration % 10 === 1) {
          scraperLogger.always(
            `[LOOP] Refreshing key list from database (iteration ${iteration})`
          )
          await this.loadExistingKeys()
        }
        ...
        scraperLogger.always(
          `[LOOP] scrapePhase returned ${newKeys.length} keys`
        )

        // Keys are now saved immediately in scrapePhase
        if (newKeys.length > 0) {
          scraperLogger.always(
            `[LOOP] [SUCCESS] ${newKeys.length} new keys were saved immediately during scraping`
          )
        } else {
        ...
        scraperLogger.always(
          `[LOOP] [STATS] Iteration ${iteration} Summary: ${newKeys.length} new keys saved immediately, ${this.stats.totalSaved} total in DB`
        )
        ...
        scraperLogger.always(
          `[LOOP] Iteration ${iteration} complete, waiting 5s before next iteration`
        )
        await delay(5000)
      } catch (error: any) {
        if (error.message?.includes('rate limit')) {
          ...
          scraperLogger.warn(
            `Rate limit hit ${this.stats.rateLimitHits} times, cooling down`
          )
    ...
    scraperLogger.always(
      `[LOOP] Scraper loop stopped after ${iteration} iterations`
    )
    this.running = false
  }

  private async scrapePhase(): Promise<
    Array<{ key: string; source: string; sourceUrl: string }>
  > {
    ...
    try {
      scraperLogger.always(
        `[SCRAPE] Starting scrape phase with batch size: ${this.config.batchSize}`
      )
      ...
      scraperLogger.always(
        `[SCRAPE] Passing ${this.seenKeys.size} existing keys to scraper to avoid duplicates`
      )
      ...
      const codeKeys = await scrapeGitHubKeys(
      ...
      scraperLogger.always(
        `[SCRAPE] Enhanced GitHub search returned ${codeKeys.length} keys (query rotation: ${queryRotation})`
      )

      // Debug: Log the actual keys returned by enhanced GitHub search
      if (codeKeys.length > 0) {
        scraperLogger.always(
          `[SCRAPE] [DEBUG] Keys returned by enhanced GitHub search:`
        )
        codeKeys.slice(0, 3).forEach((key, i) => {
          scraperLogger.always(
            `[SCRAPE]   ${i + 1}. ${key.key.substring(0, 25)}... from ${key.source}`
          )
        })
      }
      ...
      scraperLogger.always(
        `[SCRAPE] Processing ${allScraped.length} keys for immediate database check...`
      )
      ...
      for (let i = 0; i < allScraped.length; i++) {
        ...
        const existsInPotential =
          potentialResults.data && potentialResults.data.length > 0
        const existsInWorking =
          workingResults.data && workingResults.data.length > 0
        ...
        if (!existsInPotential && !existsInWorking) {
          ...
          scraperLogger.always(
            `[SCRAPE] [NEW] Key #${newKeys.length}: ${item.key.substring(0, 20)}... (not in DB)`
          )
          ...
          if (duplicatesInThisBatch <= 3) {
            scraperLogger.always(
              `[SCRAPE] [DUPLICATE] #${duplicatesInThisBatch}: ${item.key.substring(0, 20)}... (already in DB)`
            )
          }
        }
      }
      ...
      scraperLogger.always(
        `[SCRAPE] [RESULT] ${newKeys.length} NEW keys saved immediately, ${duplicatesInThisBatch} duplicates`
      )
      ...
      if (newKeys.length > 0) {
        scraperLogger.always(
          `[SCRAPE] [SUCCESS] Found and saved ${newKeys.length} NEW keys immediately, ${duplicatesInThisBatch} duplicates skipped`
        )
      } else if (duplicatesInThisBatch > 0) {
        scraperLogger.always(
          `[SCRAPE] Found 0 new keys, ${duplicatesInThisBatch} duplicates (all already in database)`
        )
      } else {
        scraperLogger.always(
          `[SCRAPE] Found 0 keys this iteration (GitHub returned no results or search queries exhausted)`
        )
      }
    } catch (error: any) {
  ...
  private async saveKeysImmediately(
    keys: Array<{ key: string; source: string; sourceUrl: string }>
  ): Promise<void> {
    if (keys.length === 0) return

    try {
      scraperLogger.always(
        `[SAVE] Saving ${keys.length} keys immediately to database...`
      )
      ...
      if (error) {
        scraperLogger.error(
          `[SAVE] [ERROR] Error saving keys: ${error.message}`
        )
        return
      }
      ...
      scraperLogger.always(
        `[SAVE] [SUCCESS] Successfully saved ${keys.length} keys to database. Total saved: ${this.stats.totalSaved}`
      )
      ...
      if (keys.length > 0) {
        keys.slice(0, 3).forEach((key, i) => {
          scraperLogger.always(
            `[SAVE]   ${i + 1}. ${key.key.substring(0, 25)}... from ${key.source}`
          )
        })
      }
    } catch (error: any) {
      scraperLogger.error(
        `[SAVE] [ERROR] Exception saving keys: ${error.message}`
      )
    }
  }
...
class KeyProcessorService {
  ...
  stop() {
    this.running = false
    processorLogger.always(
      `Stopped. Processed: ${this.stats.totalProcessed}, valid: ${this.stats.validKeys}`
    )
  }
  ...
  private async processLoop() {
    ...
    while (this.running) {
      ...
      try {
        processorLogger.always(
          `[PROCESS] Fetching unvalidated keys (batch size: ${this.config.batchSize})`
        )
        ...
        if (error) {
          processorLogger.error(
            `[PROCESS] Error fetching keys: ${error.message}`
          )
          processorLogger.error(
            `[PROCESS] Error details: ${JSON.stringify(error)}`
          )
          await delay(10000)
          continue
        }
        ...
        if (!pendingKeys || pendingKeys.length === 0) {
          processorLogger.always(
            `[PROCESS] No unvalidated keys found, waiting ${this.config.cycleCooldown / 1000}s`
          )
        ...
        processorLogger.always(
          `[PROCESS] Found ${pendingKeys.length} unvalidated keys to process`
        )
        ...
        for (let i = 0; i < pendingKeys.length; i++) {
          ...
          processorLogger.always(
            `[PROCESS] Validating key ${i + 1}/${pendingKeys.length}: ${keyRecord.api_key.substring(0, 12)}...`
          )
          await this.processKey(keyRecord)
          processorLogger.always(
            `[PROCESS] Validation ${i + 1}/${pendingKeys.length} complete`
          )
        ...
        processorLogger.always(
          `[PROCESS] Stats: ${this.stats.totalProcessed} processed, ${this.stats.validKeys} valid, ${this.stats.invalidKeys} invalid`
        )
  ...
  private async processKey(keyRecord: any) {
    ...
    try {
      processorLogger.always(
        `[VALIDATE] Starting validation for key: ${keyPreview}...`
      )
      ...
      processorLogger.always(
        `[VALIDATE] Validation complete - isValid: ${validationResult.isValid}, models: ${validationResult.totalModelsAccessible}/${validationResult.totalModelsTested}`
      )

      // Store validation result (this moves valid/quota_exceeded keys to working_gemini_keys)
      processorLogger.always(
        `[VALIDATE] Storing validation result to database...`
      )
      ...
      if (
        ...
        validationResult.status?.includes('quota')
      ) {
        ...
        const status =
          ...
          validationResult.status === 'quota_exceeded'
            ...
            : 'VALID'
        processorLogger.always(
          `[VALIDATE] [SUCCESS] Key is ${status} - Moved to working_gemini_keys table`
        )
        if (
          validationResult.status === 'quota_reached' ||
          validationResult.status === 'quota_exceeded'
        ...
        processorLogger.warn(
          `[VALIDATE] [INVALID] Key is INVALID - Marked as validated but not added to working keys`
        )
      }

      processorLogger.always(
        `[VALIDATE] Key ${keyPreview}... processed successfully`
      )
    } catch (error: any) {
      this.stats.errors++
      processorLogger.error(
        `[VALIDATE] [ERROR] Error processing key ${keyPreview}...: ${error.message}`
      )
...
class KeyValidatorService {
  ...
  stop() {
    this.running = false
    validatorLogger.always(
      `Stopped. Validated: ${this.stats.totalProcessed}, deleted: ${this.stats.keysDeleted}`
    )
  }
  ...
  private async validationLoop() {
    ...
    while (this.running) {
      ...
      try {
        ...
        validatorLogger.always(
          `[REVALIDATOR] Fetching keys needing revalidation...`
        )
        const { data: allKeys, error } = await this.supabase
          .from('working_gemini_keys')
          .select('id, api_key, status, last_validated_at, next_check_at')
          .or(
            `next_check_at.is.null,next_check_at.lte.${new Date().toISOString()}`
          )
        ...
        if (error) {
          validatorLogger.error(
            `[REVALIDATOR] Error fetching keys: ${error.message}`
          )
          await delay(30000)
          continue
        }
        ...
        if (!allKeys || allKeys.length === 0) {
          validatorLogger.always(
            `[REVALIDATOR] No keys need revalidation yet, waiting ${this.config.cycleCooldown / 1000}s`
          )
          await delay(this.config.cycleCooldown)
          continue
        }
        ...
        validatorLogger.always(
          `[REVALIDATOR] Found ${allKeys.length} keys needing revalidation`
        )
        ...
        for (let batchIdx = 0; batchIdx < batches.length; batchIdx++) {
          ...
          validatorLogger.always(
            `[REVALIDATOR] Processing batch ${batchIdx + 1}/${batches.length} (${batch.length} keys)`
          )
          ...
          await Promise.allSettled(
            batch.map(async (keyRecord, i) => {
              ...
              this.stats.lastActivity = new Date()
              validatorLogger.always(
                `[REVALIDATOR] Revalidating key ${i + 1}/${batch.length}: ${keyRecord.api_key.substring(0, 12)}...`
              )
          ...
          validatorLogger.always(
            `[REVALIDATOR] Batch ${batchIdx + 1}/${batches.length} complete`
          )
        }

        validatorLogger.always(`[REVALIDATOR] === Cycle ${cycle} Complete ===`)
        validatorLogger.always(
          `[REVALIDATOR] Stats: ${this.stats.totalProcessed} processed, ${this.stats.stillValid} still valid, ${this.stats.becameInvalid} became invalid`
        )

        if (!this.config.continuousValidation) break

        validatorLogger.always(
          `[REVALIDATOR] Waiting ${this.config.cycleCooldown / 1000}s until next cycle`
        )
        await delay(this.config.cycleCooldown)
      } catch (error: any) {
    ...
    try {
      ...
      if (isValid && !isQuotaExceeded) {
        ...
        if (this.config.deleteInvalidKeys) {
          ...
          validatorLogger.always(
            `[REVALIDATOR] Deleted invalid key ${keyPreview}...`
          )
        } else {
          validatorLogger.always(
            `[REVALIDATOR] Keeping invalid key ${keyPreview}... (deleteInvalidKeys=false)`
          )
        }
      }
    } catch (error: any) {
      validatorLogger.error(
        `[REVALIDATOR] Error validating key ${keyPreview}...: ${error.message}`
      )
    }
  }
...
export function getValidatorService(
