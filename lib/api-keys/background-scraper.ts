...
class BackgroundScraper {
  ...
  async start(options?: {
    ...
    logger.always(
      `[BACKGROUND] Starting background scraper with ${intervalMinutes} minute intervals`
    )
    ...
    try {
      ...
      const results = await scrapeGitHubKeys(
        ...
        options?.onProgress ||
          (progress => {
            logger.always(
              `[BACKGROUND] Progress: ${progress.found} keys found, ${progress.processed}/${progress.total} processed`
            )
          }),
        {
      ...
      logger.always(
        `[BACKGROUND] Scrape task completed: ${results.length} keys found`
      )
    } catch (error) {
