  ...
  setInterval(
    () => {
      ...
      if (status.scraper.running && status.scraper.stats) {
        ...
        logger.always(
          `Scraper: scraped=${s.totalScraped}, saved=${s.totalSaved}, phase=${s.currentPhase}`
        )
      }
      ...
      if (status.processor.running && status.processor.stats) {
        ...
        logger.always(
          `Processor: processed=${p.totalProcessed}, valid=${p.validKeys}, invalid=${p.invalidKeys}`
        )
      }
      ...
      if (status.validator.running && status.validator.stats) {
        ...
        logger.always(
          `Validator: validated=${v.totalProcessed}, deleted=${v.keysDeleted}`
        )
      }
    },
