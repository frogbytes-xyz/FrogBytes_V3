...
class ContinuousValidator {
  ...
  async start(options?: {
    ...
    logger.always(
      `[CONTINUOUS] Starting continuous validator with ${intervalMinutes} minute intervals`
    )
  ...
  private async runValidationTask(options?: {
    ...
    try {
      ...
      const unvalidatedKeys = await getPendingKeysForValidation(
      ...
      logger.always(
        `[CONTINUOUS] Found ${unvalidatedKeys.length} unvalidated keys`
      )
      ...
      for (const key of unvalidatedKeys) {
        ...
        try {
          ...
          if (validationResult.isValid) {
            progress.valid++
            logger.always(
              `[CONTINUOUS] [VALID] Valid key: ${key.key.substring(0, 12)}...`
            )
          } else {
            progress.invalid++
            logger.always(
              `[CONTINUOUS] [INVALID] Invalid key: ${key.key.substring(0, 12)}...`
            )
          }
        } catch (error) {
          progress.errors++
          logger.error(
            `[CONTINUOUS] Validation error for ${key.key.substring(0, 12)}...:`,
            error
          )
        }
      ...
      logger.always(
        `[CONTINUOUS] Validation task completed: ${progress.valid} valid, ${progress.invalid} invalid, ${progress.errors} errors`
      )
    } catch (error) {
