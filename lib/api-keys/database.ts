...
export async function getPendingKeysForValidation(
  ...
  const { data, error } = await supabase
    ...
    .select(
      'id, key:api_key, source, sourceUrl:source_url, metadata, foundAt:found_at, validated'
    )
    .eq('validated', false)
...
export async function storeValidationResult(
  ...
  const isQuotaExceeded =
    ...
    (typeof validationResult.quotaRemaining === 'number' &&
      validationResult.quotaRemaining === 0)
  ...
  const status: 'quota_exceeded' | 'valid' = isQuotaExceeded
    ? 'quota_exceeded'
    : 'valid'
  ...
  const canProcessAudio = validationResult.capabilities.some(
    c =>
      c.isAccessible &&
      (c.features?.includes('audio') || c.features?.includes('native-audio'))
  )
  ...
  if (insertError) {
    throw new Error(
      `Failed to store in working_gemini_keys: ${insertError.message}`
    )
  }
 ...
 */
export async function storeValidationError(
  apiKey: string,
  _error: string
): Promise<void> {
...
export async function getValidationStats(): Promise<{
  ...
  const validationProgress =
    totalScraped > 0 ? (totalValidated / totalScraped) * 100 : 0

  const totalValid = workingKeys.filter(k => k.status === 'valid').length
  const totalQuotaExceeded = workingKeys.filter(
    k => k.status === 'quota_exceeded'
  ).length
  ...
  const { error } = await supabase
    ...
    .update({
      ...
      next_check_at: new Date(
        Date.now() + nextCheckMinutes * 60000
      ).toISOString()
    })
    .eq('api_key', apiKey)
 ...
 */
export async function getEnrichedScrapedKeys(
  limit = 50,
  offset = 0
): Promise<any[]> {
  const { data, error } = await supabase
