...
export function loadVideoDownloadConfig(): VideoDownloadConfig {
  const rawConfig = {
    ...
    cookieEncryptionKey:
      process.env.COOKIE_ENCRYPTION_KEY || generateDefaultEncryptionKey(),
  ...
  try {
    ...
    if (error instanceof z.ZodError) {
      ...
      throw new Error(
        `Video download configuration validation failed:\n${errorMessages.join('\n')}`
      )
    }
    throw error
  }
...
function generateDefaultEncryptionKey(): string {
  if (process.env.NODE_ENV === 'production') {
    throw new Error(
      'COOKIE_ENCRYPTION_KEY environment variable is required in production. Please set a secure 32+ character encryption key'
    )
  }
  ...
  logger.warn(
    '[WARNING] Using auto-generated encryption key for development. Set COOKIE_ENCRYPTION_KEY in production!'
  )
  return result
}
 ...
 */
export async function validateDependencies(): Promise<{
  isValid: boolean
  errors: string[]
}> {
  const errors: string[] = []
  ...
  try {
    ...
    errors.push(
      'Redis is not accessible. Please ensure Redis is running and accessible.'
    )
  }
