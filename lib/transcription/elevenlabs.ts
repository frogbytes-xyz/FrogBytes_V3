...
export async function transcribeAudio(
  ...
  try {
    logger.info(
      `[ElevenLabs] Transcribing with ${shouldUseAuth ? 'authenticated' : 'free'} endpoint`
    )
    ...
    logger.info(
      `[ElevenLabs] Using ${shouldUseAuth ? 'authenticated' : 'FREE'} endpoint`
    )
    ...
    const result = await new Promise<any>((resolve, reject) => {
      const req = https.request(options, (res: any) => {
        ...
        res.on('end', () => {
          logger.info(
            `[ElevenLabs] Response status: ${res.statusCode} ${res.statusMessage}`
          )
    ...
    if (!result && shouldUseAuth && useAuth !== false) {
      logger.warn(
        '[ElevenLabs] Authenticated endpoint failed, trying free endpoint...'
      )
    ...
    logger.info(
      `[ElevenLabs] Language: ${result.language_code} (${(result.language_probability * 100).toFixed(1)}% confidence)`
    )
    logger.info(
      `[ElevenLabs] Word count: ${result.words?.length || 0} words with timestamps`
    )
    ...
    if (!text) {
      logger.error(
        '[ElevenLabs] No text in response',
    ...
    const wordCount =
      result.words?.filter((w: any) => w.type === 'word').length ||
