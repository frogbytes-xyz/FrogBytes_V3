...
export default function RegisterPage() {
  ...
  const getPasswordStrength = (pwd: string): { strength: string; color: string; width: string } => {
    ...
    if (score <= 4) return { strength: 'Medium', color: 'bg-[#fbbf24]', width: '66%' };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(data.details?.[0] || data.error || 'Registration failed');
      }
    ...
    <>
      ...
      <main className="min-h-screen flex items-center justify-center p-4 pt-24">
        ...
        <Card className="border bg-card text-card-foreground">
          <CardHeader>
            <CardTitle className="text-lg font-medium leading-none tracking-tight mb-2">Sign Up</CardTitle>
            ...
            <form className="space-y-4" onSubmit={handleSubmit}>
              ...
              <div className="space-y-2">
                <div className="flex items-center gap-2">
                  ...
                  <Badge variant="tag-rounded" className="text-[10px]">Optional</Badge>
                </div>
                <Input
                  id="university"
                  name="university"
              ...
              <div className="space-y-2">
                ...
                <div className="relative">
                  ...
                  >
                    <span className="text-xs text-muted-foreground">{showConfirmPassword ? 'Hide' : 'Show'}</span>
                  </button>
                </div>
