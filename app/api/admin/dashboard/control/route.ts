...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!['scraper', 'validator', 'revalidator'].includes(service)) {
      return NextResponse.json(
        { success: false, error: 'Invalid service. Must be "scraper" or "validator"' },
        { status: 400 }
      );
    }
    ...
    if (!['start', 'restart'].includes(action)) {
      return NextResponse.json(
        { success: false, error: 'Invalid action. Must be "start" or "restart"' },
        { status: 400 }
      );
    }

    // Trigger the appropriate cron job endpoint
    const endpoint = service === 'scraper'
