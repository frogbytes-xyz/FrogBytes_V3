...
export async function GET(request: NextRequest) {
  try {
    ...
    if (isValid === 'true' || isValid === 'false') {
      ...
      const onlyValid = (wk || []).filter(w => w.status === 'valid').map(w => w.api_key);
      validKeys = onlyValid;
    }
    ...
    if (isValid === 'false' && validKeys) {
      ...
      potentialQuery = potentialQuery.eq('validated', true).not('api_key', 'in', `(${validKeys.map(v => `'${v}'`).join(',') || "''"})`);
    }
    ...
    let workingMap = new Map<string, any>();
    if (apiKeys.length > 0) {
    ...
    if (apiKeys.length > 0) {
      ...
      (logs || []).forEach((log: any) => {
        ...
        if (outcome) {
          arr.push({ outcome, at: log.created_at, prev: details.previousStatus || null, next: details.newStatus || null });
    ...
    // Summary statistics
    const [pendingCountRes, processedCountRes, validCountRes, quotaCountRes] = await Promise.all([
      supabase.from('potential_keys').select('id', { count: 'exact', head: true }).eq('validated', false),
      supabase.from('potential_keys').select('id', { count: 'exact', head: true }).eq('validated', true),
      supabase.from('working_gemini_keys').select('id', { count: 'exact', head: true }).eq('status', 'valid'),
      supabase.from('working_gemini_keys').select('id', { count: 'exact', head: true }).eq('status', 'quota_exceeded'),
    ]);
