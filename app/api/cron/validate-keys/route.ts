...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    for (const keyRecord of pendingKeys) {
      ...
      try {
        await logger.info(
          `Validating key ${results.validated + 1}/${pendingKeys.length}`,
          {
        ...
        if (validationResult.isValid) {
          ...
          await logger.success(
            `Key ${results.validated}/${pendingKeys.length} is valid: ${validationResult.totalModelsAccessible}/${validationResult.totalModelsTested} models accessible`,
            {
              keyPreview: keyPreview + '...',
              modelsAccessible: validationResult.totalModelsAccessible,
              modelsTested: validationResult.totalModelsTested,
              bestModel: validationResult.capabilities.find(c => c.isAccessible)
                ?.modelName
            }
          )
        } else {
          results.invalid++
          await logger.warn(
            `Key ${results.validated}/${pendingKeys.length} is invalid`,
            {
              keyPreview: keyPreview + '...',
    ...
    await logger.success(
      `Validation completed: ${results.valid} valid, ${results.invalid} invalid out of ${results.validated} total`,
      results
    )
    ...
    for (const keyRecord of pendingKeys) {
      ...
      try {
        ...
        if (validationResult.isValid) {
          results.valid++
          await logger.success(
            `Key ${results.validated}/${pendingKeys.length} is valid`,
            {
              keyPreview: keyPreview + '...',
              modelsAccessible: validationResult.totalModelsAccessible
            }
        ...
        results.errors++
        await logger.error(
          `Validation failed for key ${results.validated + 1}`,
          {
            keyPreview: keyPreview + '...',
            error: error.message
          }
