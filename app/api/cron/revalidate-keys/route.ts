...
async function runRevalidation(
  ...
  for (let batchIdx = 0; batchIdx < chunks.length; batchIdx++) {
    ...
    await Promise.allSettled(
      batch.map(async (rec: any, j: number) => {
        const i = batchIdx * concurrency + j
        const preview = rec.api_key.substring(0, 12)
        try {
          await logger.info(`Revalidate ${i + 1}/${keys.length}`, {
            keyPreview: preview + '...'
          })
          const validation = await validateGeminiKey(rec.api_key)
          results.processed++
          const isQuota =
            validation.status === 'quota_reached' ||
            validation.status === 'quota_exceeded' ||
            (typeof validation.quotaRemaining === 'number' &&
              validation.quotaRemaining === 0)
          ...
          results.errors++
          await logger.error(`Error revalidating ${preview}...`, {
            error: error.message
          })
        }
      })
...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const concurrency =
      parseInt(process.env.REVALIDATOR_CONCURRENCY || '5', 10) || 5
    ...
    await logger.error('Revalidation failed', {
      error: error.message,
      stack: error.stack
    })
