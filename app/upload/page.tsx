...
export default function UploadPage() {
  ...
  const handleMetadataComplete = async (metadata: LibraryMetadata) => {
    ...
    try {
      ...
      if (metadata.documentType)
        updateData.document_type = metadata.documentType
      if (metadata.fileCategory)
        updateData.file_category = metadata.fileCategory
      ...
      if (metadata.academicYear)
        updateData.academic_year = metadata.academicYear
      if (metadata.lectureNumber)
        updateData.lecture_number = metadata.lectureNumber
      ...
      if (metadata.difficultyLevel)
        updateData.difficulty_level = metadata.difficultyLevel
      if (metadata.tags && metadata.tags.length > 0)
        updateData.tags = metadata.tags
      if (metadata.makePublic !== undefined)
        updateData.is_public = metadata.makePublic

      // Mark metadata as complete if key fields are present
      updateData.metadata_complete = !!(
        metadata.title &&
        metadata.university &&
        metadata.subject
      )
  ...
  const startProcessing = async (fileId?: string) => {
    ...
    try {
      ...
      if (!transcribeResponse.ok) {
        ...
        throw new Error(
          errorData.details?.[0] || errorData.error || 'Transcription failed'
        )
      }
      ...
      if (!summarizeResponse.ok) {
        ...
        throw new Error(
          errorData.details?.[0] || errorData.error || 'Summarization failed'
        )
      }
      ...
      if (uploadMetadata.documentType)
        updateData.document_type = uploadMetadata.documentType
      if (uploadMetadata.fileCategory)
        updateData.file_category = uploadMetadata.fileCategory
      if (uploadMetadata.university)
        updateData.university = uploadMetadata.university
      if (uploadMetadata.courseCode)
        updateData.course_code = uploadMetadata.courseCode
      if (uploadMetadata.courseName)
        updateData.course_name = uploadMetadata.courseName
      if (uploadMetadata.subject) updateData.subject = uploadMetadata.subject
      if (uploadMetadata.professor)
        updateData.professor = uploadMetadata.professor
      if (uploadMetadata.semester) updateData.semester = uploadMetadata.semester
      if (uploadMetadata.academicYear)
        updateData.academic_year = uploadMetadata.academicYear
      if (uploadMetadata.lectureNumber)
        updateData.lecture_number = uploadMetadata.lectureNumber
      if (uploadMetadata.lectureDate)
        updateData.lecture_date = uploadMetadata.lectureDate
      if (uploadMetadata.language) updateData.language = uploadMetadata.language
      if (uploadMetadata.difficultyLevel)
        updateData.difficulty_level = uploadMetadata.difficultyLevel
      const maybeTags = uploadMetadata.tags
      if (
        maybeTags &&
        Array.isArray(maybeTags) &&
        (maybeTags as string[]).length > 0
      ) {
        updateData.tags = maybeTags as string[]
      }
      if (uploadMetadata.makePublic !== undefined)
        updateData.is_public = uploadMetadata.makePublic

      // Mark metadata as complete if key fields are present
      updateData.metadata_complete = !!(
        uploadMetadata.title &&
        uploadMetadata.university &&
        uploadMetadata.subject
      )
      ...
      if (updateError) {
        logger.error(
          'Failed to update summary with PDF URL and metadata',
          updateError
        )
  ...
  const handleUploadError = (error: string) => {
    ...
    if (isAuthError) {
      logger.info(
        'Authentication error detected - keeping FileUpload visible for cookie extraction'
      )
      ...
      <div className="container max-w-5xl pt-28 pb-24 px-4">
        {processingStep !== 'collecting-metadata' && (
          <div className="mb-16 text-center">
            <h1 className="text-4xl md:text-5xl font-normal mb-4 text-foreground">
              Upload Lecture
            </h1>
            <p className="text-lg text-muted-foreground max-w-2xl mx-auto">
              Upload an audio or video file of your lecture to generate an
              AI-powered summary
            </p>
          </div>
        )}
        ...
        {processingStep === 'idle' && (
          <div className="mt-16">
            <h2 className="text-center text-2xl font-normal mb-3 text-foreground">
              How it works
            </h2>
              ...
              {[
                {
                  ...
                  title: 'Upload',
                  description:
                    'Upload your lecture recording (audio or video file)'
                },
                {
                  number: 2,
                  title: 'Transcribe',
                  description: 'AI transcribes the content using ElevenLabs'
                },
                {
                  number: 3,
                  title: 'Summarize',
                  description: 'Generate structured summary with Gemini AI'
                },
                {
                  number: 4,
                  title: 'Learn',
                  description: 'View PDF and use AI Copilot for questions'
                }
              ].map((step, index) => (
