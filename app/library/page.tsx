...
export default function LibraryPage() {
  ...
  const {
    handleVote: handleVoteWithDebounce,
    voteStates,
    clearPendingVotes
  } = useVoting({
  ...
  const loadSummaries = useCallback(async () => {
    try {
      ...
      const { data, error: fetchError } = await supabase
        ...
        .eq('is_public', true)
        .not('pdf_url', 'is', null)
        .order(sortBy === 'reputation' ? 'reputation_score' : 'created_at', {
          ascending: false
        })
        .limit(100)
    ...
    try {
      ...
      if (data) {
        const uniqueUniversities = [
          ...new Set(data.map((d: any) => d.university).filter(Boolean))
        ] as string[]
        const uniqueSubjects = [
          ...new Set(data.map((d: any) => d.subject).filter(Boolean))
        ] as string[]
  ...
  const handleVote = useCallback(
    (summaryId: string, vote: 1 | -1) => {
      ...
      const currentVote: 1 | -1 | null =
        summary.user_vote === 1 ? 1 : summary.user_vote === -1 ? -1 : null
  ...
  const filteredSummaries = (() => {
    ...
    if (selectedDocumentType !== 'all')
      filters.documentType = selectedDocumentType
    if (selectedDifficultyLevel !== 'all')
      filters.difficultyLevel = selectedDifficultyLevel
          ...
          <div className="space-y-6">
            ...
            <div className="flex flex-wrap items-center gap-4">
              ...
              <span className="text-sm text-muted-foreground ml-auto font-medium">
                ...
                {filteredSummaries.length === 1 ? 'result' : 'results'}
              </span>
            </div>
          </div>
        </div>
              ...
              {summaries.length === 0 ? (
                <>
                  ...
                  <p className="text-sm text-muted-foreground max-w-md mx-auto">
                    Be the first to contribute! Upload your lecture notes and
                        ...
                        <div className="pt-3 border-t flex items-center justify-between gap-2">
                          <div className="flex items-center gap-1">
                            <button
                              ...
                              disabled={
                                !currentUser || voteStates[summary.id]?.isVoting
                              }
                              ...
                              title={
                                !currentUser ? 'Sign in to vote' : 'Upvote'
                              }
                            >
                              <svg
                                className="w-4 h-4"
                            ...
                            <button
                              ...
                              title={
                                !currentUser ? 'Sign in to vote' : 'Downvote'
                              }
                            >
                              <svg
                                className="w-4 h-4"
                          ...
                          <div className="flex items-start justify-between gap-4">
                            <div className="flex-1 min-w-0 space-y-3">
                              ...
                              <div className="flex items-center flex-wrap gap-2 text-sm text-muted-foreground">
                                ...
                                <span>•</span>
                                <span>
                                  By {summary.user?.full_name || 'Anonymous'}
                                </span>
                                <span>•</span>
                                <span>
                                  {new Date(
